...
export function ItemVeiculoCarousel(veiculo: VeiculoSchema) {
    function createImagens(imagens: VeiculoImagemSchema[]): string {
        return imagens
            .map(
                (imagem, index) => `
    ...
    const imagens =
        veiculo?.imagens?.filter(
            (imagem: VeiculoImagemSchema) => imagem.status == "ativo"
        ) || [];
